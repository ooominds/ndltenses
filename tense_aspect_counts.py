...
TOP = "/rds/projects/d/divjakd-ooo-machines/Users/tekevwe/English_tense/Data_preparation/Data_shared"
#TOP = r"D:\work\OoOM\nlp_aspect\English_tense\Data_preparation\Data_shared"
#TOP = os.path.join("D:", "work", "OoOM", "nlp_aspect", "English_tense", "Data_preparation", "Data_shared")
#WD = TOP + 'Prepare_events
#os.chdir(WD)
...
### Define file paths
TENSES_ANNOTATED_NOINF = os.path.join(TOP, "NoDoparallel", "tenses_annotated_noinf.csv")
TENSES_ANNOTATED_NOINF_CLEAN = os.path.join(TOP, "NoDoparallel", "tenses_annotated_noinf_clean.csv")

TENSES =  os.path.join(TOP, "tenses_annotated_noinf_clean.csv")
TENSES_WITH_INF = os.path.join(TOP, "tenses_annotated_withinf.csv")
TENSES_WITH_INF_NEW = os.path.join(TOP, "tenses_annotated_withinf_new.csv")
TENSES_ONE_SENT_PER_VERB = os.path.join(TOP, "tenses_annotated_one_sent_per_verb.csv")
TENSES_ONE_SENT_PER_VERB_WITH_MODALS = os.path.join(TOP, "tenses_annotated_one_sent_per_verb_withmodals.csv")
TENSES_ONE_SENT_PER_VERB_READY = os.path.join(TOP, "tenses_annotated_one_sent_per_verb_ready.csv")
TENSES_ONE_SENT_PER_VERB_READY_GZ = os.path.join(TOP, "tenses_annotated_one_sent_per_verb_ready.csv.gz")
TENSES_ONE_SENT_PER_VERB_SHUF_GZ = os.path.join(TOP, "tenses_annotated_one_sent_per_verb_shuffeled.csv.gz")
TENSES_ONE_VERB = os.path.join(TOP, "tenses_annotated_oneverb.csv")
TENSES_ONE_VERB_READY_GZ = os.path.join(TOP, "tenses_annotated_oneverb_ready.csv.gz")
TENSES_ONE_VERB_SHUF_GZ = os.path.join(TOP, "tenses_annotated_oneverb_shuffeled.csv.gz")
AE2BE_LIST = os.path.join(TOP, "Data","List_AE2BE.csv")
INFINITIVE_CORR_LIST = os.path.join(TOP, "Data","Infinitive_corrections_freq10.csv")
...
### Load the data
tenses = pd.read_csv(TENSES_ANNOTATED_NOINF)
...
tenses.to_csv(TENSES_WITH_INF, index = False)
...
# Export the dataset
tenses.to_csv(TENSES_WITH_INF_NEW, index = False)
...
# => 138425 SENTENCES WITH NO TENSE
...
with open(TENSES_ONE_SENT_PER_VERB_WITH_MODALS, mode = 'w') as o:
    ...
    for i in range(0, nR):
        ...
        for j in range(1, (nV+1)):
            if str(tenses.iloc[i]["".join(['Tense', str(j)])]) != 'nan':
                # Write the row
...
# Load the data
tenses = pd.read_csv(TENSES_ONE_SENT_PER_VERB_WITH_MODALS)
...
def convert_mainverb_AE2BE(word):
...
tenses["Sentence_BE"] = tenses["Sentence"].apply(lambda s: convert_AE2BE(s))

# Convert the verbforms in the dataframe
tenses["VerbForm_BE"] = tenses["VerbForm"].apply(lambda s: convert_AE2BE(s))

# Convert the verbforms in the dataframe
tenses["MainVerb_BE"] = tenses["MainVerb"].apply(lambda s: convert_mainverb_AE2BE(s))
...
# Number of cases
tenses.shape
# (8031972, 9) # after removing those empty modals
...
# Export the dataset
tenses.to_csv(TENSES_ONE_SENT_PER_VERB, index = False)
...
# Load the data
tenses = pd.read_csv(TENSES_ONE_SENT_PER_VERB, keep_default_na = False)
...
# Export the dataset
tenses.to_csv(TENSES_ONE_SENT_PER_VERB_READY_GZ, compression='gzip', index = False)
...
### Load the data
start = time.time()
tenses = pd.read_csv(TENSES_ONE_SENT_PER_VERB_READY_GZ, compression='gzip')
...
# Export the dataset
tenses1.to_csv(TENSES_ONE_VERB_READY_GZ, compression='gzip', index = False)
...
for id in SentID_list:
    count_sentid[id] += 1
#count_sentid.most_common(10)
# [(7045, 10), (4119, 9), (4744, 9), (7200, 9), (114, 8), (201, 8), (1244, 8), (5042, 8), (5044, 8), (5059, 8)]
...
# Export the dataset
tenses.to_csv(TENSES_ONE_SENT_PER_VERB_SHUF_GZ, compression='gzip', index = False)
...
# Export the dataset
tenses1.to_csv(TENSES_ONE_VERB_SHUF_GZ, compression='gzip', index = False)
