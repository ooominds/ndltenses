...
from ndl_tense.simulations.evaluation import recall, precision, f1score
from ndl_tense.simulations.preprocessing import df_to_gz, IndexedFile, prepare_embedding_matrix, extract_embedding_dim
...
class generator_textfile_FNN(keras.utils.all_utils.Sequence):
...
class generator_df_FNN(keras.utils.all_utils.Sequence):
...
class NDLmodel():
    ...
    #def save(self, path):
    #    with h5py.File("%s.h5"%(path), 'w') as f:
    #        f.create_dataset(self.weights)


def train_NDL(data_train, data_valid, cue_index, outcome_index,
    ...
    from ndl_tense.simulations.evaluation import activations_to_predictions, predict_outcomes_NDL
    from ndl_tense.simulations.preprocessing import df_to_gz
...
def load_NDL_model(path):
    model = NDLmodel(xr.open_dataarray(path))
    return(model)
